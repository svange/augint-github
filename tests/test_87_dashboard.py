...
from gh_secrets_and_vars_async.dashboard import state
from gh_secrets_and_vars_async.dashboard.app import DashboardApp
from gh_secrets_and_vars_async.dashboard.layouts import (
    get_layout,
    list_layouts,
    register_layout,
)
from gh_secrets_and_vars_async.dashboard.state import (
    FILTER_MODES,
    SORT_MODES,
    AppState,
    RepoTeamInfo,
...
from gh_secrets_and_vars_async.dashboard.themes import get_theme, list_themes
...
class TestDashboardCLI:
    ...
    def test_dashboard_bad_theme(self):
        ...
        with (
            patch(
                "gh_secrets_and_vars_async.dashboard.cmd.load_env_config",
                return_value=("repo", "account", "tok"),
            ),
            patch("gh_secrets_and_vars_async.dashboard.cmd.get_github_client"),
        ):
    ...
    @patch("gh_secrets_and_vars_async.dashboard.app.run_dashboard", side_effect=KeyboardInterrupt)
    @patch("gh_secrets_and_vars_async.common.get_github_repo")
    @patch("gh_secrets_and_vars_async.dashboard.cmd.load_env_config")
    @patch("gh_secrets_and_vars_async.dashboard.cmd.get_github_client")
    ...
    @patch("gh_secrets_and_vars_async.dashboard.app.run_dashboard", side_effect=KeyboardInterrupt)
    @patch("gh_secrets_and_vars_async.dashboard.cmd.list_repos")
    @patch("gh_secrets_and_vars_async.dashboard.cmd.load_env_config")
    @patch("gh_secrets_and_vars_async.dashboard.cmd.get_github_client")
...
class TestLayoutRegistry:
    ...
    def test_register_new_layout(self):
        ...
        try:
            ...
            from gh_secrets_and_vars_async.dashboard.layouts import _LAYOUTS
...
class TestDashboardApp:
    ...
    def test_app_does_not_fetch_usage_in_init(self):
        ...
        with patch(
            "gh_secrets_and_vars_async.dashboard.app.fetch_all_usage", return_value=[]
        ) as mock_fetch:
    ...
    def test_app_records_usage_error_after_mount(self):
        async def run():
            with patch(
                "gh_secrets_and_vars_async.dashboard.app.fetch_all_usage",
...
class TestDashboardActions:
    ...
    def test_o_opens_browser(self):
        async def run():
            app = DashboardApp(repos=[], skip_refresh=True)
            _seed_state(app)
            with patch("gh_secrets_and_vars_async.dashboard.app.webbrowser.open") as m:
                async with app.run_test() as pilot:
                    await pilot.pause()
                    await pilot.press("o")
                    await pilot.pause()
...
class TestRepoCardRender:
    def _spec(self):
        from gh_secrets_and_vars_async.dashboard.widgets.repo_card import RepoCard
...
class TestDrillDown:
    def test_drilldown_renders_findings(self):
        from gh_secrets_and_vars_async.dashboard.screens.drilldown import (
            DrillDownScreen,
        )
    ...
    def test_drilldown_open_browser(self):
        ...
        screen = DrillDownScreen(_health())
        with patch("gh_secrets_and_vars_async.dashboard.screens.drilldown.webbrowser.open") as m:
...
class TestStateHelpers:
    def test_selected_health_returns_none_when_empty(self):
        from gh_secrets_and_vars_async.dashboard.state import selected_health
    ...
    def test_remember_repo_teams(self):
        from gh_secrets_and_vars_async.dashboard.state import remember_repo_teams
...
class TestBootstrapCache:
    def test_bootstrap_no_cache(self):
        from gh_secrets_and_vars_async.dashboard.state import bootstrap_from_cache

        s = AppState()
        with patch("gh_secrets_and_vars_async.dashboard.state.load_cache", return_value={}):
    ...
    def test_bootstrap_with_cache(self):
        ...
        with (
            patch(
                "gh_secrets_and_vars_async.dashboard.state.load_cache",
                return_value={"org/x": status},
            ),
            patch(
                "gh_secrets_and_vars_async.dashboard.state.load_health_cache",
                return_value={},
            ),
        ):
...
class TestAppMisc:
    def test_card_selected_message_updates_state(self):
        async def run():
            ...
            async with app.run_test() as pilot:
                ...
                from gh_secrets_and_vars_async.dashboard.widgets.repo_card import (
                    RepoCard,
                )
    ...
    def test_card_actions_opens_browser(self):
        async def run():
            ...
            async with app.run_test() as pilot:
                ...
                with patch("gh_secrets_and_vars_async.dashboard.app.webbrowser.open_new_tab") as m:
    ...
    def test_run_dashboard_invokes_app_run(self):
        from gh_secrets_and_vars_async.dashboard.app import run_dashboard

        with patch("gh_secrets_and_vars_async.dashboard.app.DashboardApp") as mock_app_cls:
            instance = MagicMock()
    ...
    def test_selected_badge_in_render(self):
        from gh_secrets_and_vars_async.dashboard.themes import get_theme
        from gh_secrets_and_vars_async.dashboard.widgets.repo_card import RepoCard
    ...
    def test_header_click_toggles_org_drawer(self):
        async def run():
            from gh_secrets_and_vars_async.dashboard.app import OrgDrawerHeader
    ...
    def test_usage_block_renders_meter(self):
        async def run():
            from gh_secrets_and_vars_async.panel_usage import UsageStats
    ...
    def test_countdown_seeded_at_mount_with_repos(self):
        async def run():
            ...
            with (
                patch(
                    "gh_secrets_and_vars_async.dashboard.app.bootstrap_from_cache",
                    return_value=False,
                ),
    ...
    def test_status_bar_auto_refresh_off_phrase(self):
        from gh_secrets_and_vars_async.dashboard.widgets.status_bar import StatusBar
    ...
    def test_sparkline_renders_block_heights(self):
        from gh_secrets_and_vars_async.dashboard.app import _sparkline
    ...
    def test_strip_dotfile_repos_filters_leading_dot(self):
        from gh_secrets_and_vars_async.dashboard.cmd import _strip_dotfile_repos
    ...
    def test_panel_usage_history_fallback(self, tmp_path, monkeypatch):
        ...
        from gh_secrets_and_vars_async import panel_usage
